...
MidiEvent makeMidiEvent(const GCMidi::RawMidi& rawEv) {
    MidiEvent ev{};
    ev.frame = rawEv.frame;
    ev.size = rawEv.size;
    ev.dataExt = nullptr;
...
GameControllerMIDIPlugin::~GameControllerMIDIPlugin() {
    ...
    GCMidi::SdlManager::getInstance().clearEventHandler(fDispatcher.get());
    sInstanceCount--;
}

GCMidi::EventDispatcher* GameControllerMIDIPlugin::dispatcher() {
    return fDispatcher.get();
}

MapperConfig::MapperPreset GameControllerMIDIPlugin::activeConfigCopy() const {
    std::lock_guard<std::mutex> lock(fConfigMutex);
    return fActiveConfig;
}

void GameControllerMIDIPlugin::setActiveConfig(const MapperConfig::MapperPreset& config) {
    {
        std::lock_guard<std::mutex> lock(fConfigMutex);
        fActiveConfig = config;
    }

    reloadMapper();
}

void GameControllerMIDIPlugin::setPlayMode(bool playMode) {
    fPlayMode = playMode;
}

void GameControllerMIDIPlugin::deactivate() {
    if (fDispatcher) {
        fDispatcher->deactivate();
    }
}
...
void GameControllerMIDIPlugin::reloadMapper() {
    MapperConfig::MapperPreset configCopy;
    {
        std::lock_guard<std::mutex> lock(fConfigMutex);
        configCopy = fActiveConfig;
    }

    auto mapper = std::make_unique<GCMidi::FlexibleMapper>();
    mapper->setPreset(configCopy);
    fDispatcher->setMapper(std::move(mapper));
}
...
void GameControllerMIDIPlugin::run(
    ...
    // Pass host MIDI through before handling controller-generated output.
    ...
    // Sync parameter changes requested by the dispatcher (e.g. D-Pad octave change).
    if (fDispatcher->sharedState().consumeBaseOctaveHostSyncPending()) {
    ...
    // Retry the MIDI event that was already popped but not accepted last callback.
    GCMidi::RawMidi rawEv;
    if (fPendingMidiOutput.has_value()) {
        rawEv = *fPendingMidiOutput;
        MidiEvent ev = makeMidiEvent(rawEv);

        if (!writeMidiEvent(ev)) {
            return;
        }
        fPendingMidiOutput.reset();
    }

    // Drain controller-generated MIDI from the dispatcher.
    while (fDispatcher->popMidi(rawEv)) {
        MidiEvent ev = makeMidiEvent(rawEv);

        if (!writeMidiEvent(ev)) {
            fPendingMidiOutput = rawEv;
            break;
        }
    }
}
